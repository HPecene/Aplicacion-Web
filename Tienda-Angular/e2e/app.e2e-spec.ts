...
describe('Starting tests for Tienda-Angular', function() {
  ...
  it('website title should be Tienda-Angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Tienda-Angular');
    })
  });

  it('network-name should be tienda@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tienda@0.0.1.bna');
    });
  });

  it('navbar-brand should be Tienda-Angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Tienda-Angular');
    });
  });
